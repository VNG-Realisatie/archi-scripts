...
function applyToConceptTypeInView(pSelection, pFunc, pFuncParam) {
	...
	$(selectedConcept.view)
		.find(selectedConcept.concept.type)
		.forEach(function (occurrence) {
			pFunc(occurrence, pFuncParam);
			i++;
		});
	console.log(
		`> ${pFunc.name} applied to ${i} concepts of type ${selectedConcept.type} in view "${selectedConcept.view.name}"`
	);
}
...
function applyToSelectionInView(pSelection, pFunc, pFuncParam) {
	...
	console.log(
		`>> ${
			pSelection.size() - pSelection.filter("element").size() - pSelection.filter("relation").size()
		} diagram-model concepts"`
	);
